...
int currTerminalWidth = 0;
int currTerminalHeight = 0;
...
int main() {
	...
	getTerminalSize(&currTerminalWidth, &currTerminalHeight);
	printf("Width: %d\nHeight: %d\n", currTerminalWidth, currTerminalHeight);
	...
	int adjustedHeight = currTerminalHeight / 3;
	int i, userInput;

	do {
		char *optionsArr[5] = {"1) Generate Keys", "2) Encrypt Text", "3) Decrypt Text", "4) About", "5) Exit program"};
		for ( i = 0; i < 5; i++) {
			moveCursor((currTerminalWidth - 14) / 2, adjustedHeight + i);
			printf("%s\n", optionsArr[i]);
		}

		moveCursor((currTerminalWidth - 10)/ 2, adjustedHeight + i);
		printf("Enter number: ");
		scanf("%d", &userInput);
		...
		switch (userInput) {
			case 1:
				generateKeys(currTerminalWidth, adjustedHeight, i);
				break;
			case 2:
				encryptText(currTerminalWidth, adjustedHeight);
				break;
			case 3:
				decryptText(currTerminalWidth, adjustedHeight);
				break;
			case 4:
				aboutProgram(currTerminalWidth, currTerminalHeight);
				break;
			case 5:
				clearScreen();
				moveCursor((currTerminalWidth - 20) / 2, adjustedHeight + 5);
				printf("Exiting program...\n");
				sleepProgram(500);
				break;
			default:
				clearScreen();
				moveCursor((currTerminalWidth - 15) / 2, adjustedHeight + 1);
				printf("Invalid input.");
				moveCursor((currTerminalWidth - 39) / 2, adjustedHeight + 2);
...
void getTerminalSize(int *width, int *height) {
	...
	currTerminalWidth = csbi.srWindow.Right - csbi.srWindow.Left + 1;
	currTerminalHeight = csbi.srWindow.Bottom - csbi.srWindow.Top + 1;
#else
	struct winsize size;
...
void waitForDone(int width, int height) {
	char done[100];
	do {
		clearLines(currTerminalHeight - 2, currTerminalHeight - 2, currTerminalWidth);
		moveCursor((currTerminalWidth - 21)/ 2, currTerminalHeight - 2);
...
char getConfirm(int width, int adjustedHeight, int offsetY) {
	...
	if (confirm != 'Y' && confirm != 'y' && confirm != 'N' && confirm != 'n') {
		moveCursor((currTerminalWidth - 37) / 2, adjustedHeight + offsetY);
...
int displayMessage(char *message[], int count, int width, int adjustedHeight) {
	int i;
	for ( i = 0; i < count; i++) {
		moveCursor((currTerminalWidth - strlen(message[i])) / 2, adjustedHeight + i);
		printf("%s\n", message[i]);
	}
...
void displayAbout(char *message[], int count, int width, int height, int offsetY) {
	for (int i = 0; i < count; i++) {
		moveCursor((currTerminalWidth - strlen(message[i])) / 2, currTerminalHeight / 4 + i + offsetY);
		printf("%s\n", message[i]);
	}
}
...
void generateKeys(int width, int adjustedHeight, int i) {
	...
	offsetY = displayMessage(msgGenerate, countGenerate, currTerminalWidth, adjustedHeight);

	do{
		clearLines(adjustedHeight + offsetY, adjustedHeight + offsetY + 1, currTerminalWidth);
		moveCursor((currTerminalWidth - 37)/ 2, adjustedHeight + offsetY);
		printf("Do you agree to save a copy of your\n");
		moveCursor((currTerminalWidth - 32)/ 2, adjustedHeight + offsetY + 1);
		printf("private and public keys? [Y/N] ");
		confirm = getConfirm(currTerminalWidth, adjustedHeight, offsetY + 1);
	...
	if (confirm == 'Y' || confirm == 'y') {
		...
		moveCursor((currTerminalWidth - 30)/ 2, adjustedHeight);
		printf("Keys generated successfully!\n");
	} else {
		moveCursor((currTerminalWidth - 25)/ 2, adjustedHeight);
		printf("Keys generation failed!\n");
	}

	waitForDone(currTerminalWidth, adjustedHeight * 3);
	clearScreen();
}
...
void encryptText(int width, int adjustedHeight) {
	...
	offsetY = displayMessage(msgEncrypt, countEncrypt, currTerminalWidth, adjustedHeight);

	do {
		clearLines(adjustedHeight + offsetY, adjustedHeight + offsetY, currTerminalWidth);
		moveCursor((currTerminalWidth - 60)/ 2, adjustedHeight + offsetY);
	...
	if (confirm == 'Y' || confirm == 'y') {
		...
		if (confirm == 'Y' || confirm == 'y') {
			...
			do {
				clearLines(adjustedHeight, adjustedHeight, currTerminalWidth);
				moveCursor((currTerminalWidth - 38)/ 2, adjustedHeight);
				...
				if (file == NULL) {
					...
					moveCursor((currTerminalWidth - 38)/ 2, adjustedHeight + offsetY + 3);
			...
			moveCursor((currTerminalWidth - 38)/ 2, adjustedHeight + 1);
			...
			moveCursor((currTerminalWidth - 30)/ 2, adjustedHeight + 1);
			printf("Function executed in: %f seconds\n", cpu_time_used);
			moveCursor((currTerminalWidth - 30)/ 2, adjustedHeight + 2);
...
void decryptText(int width, int adjustedHeight) {
	...
	offsetY = displayMessage(msgDecrypt, countDecrypt, currTerminalWidth, adjustedHeight);

	do {
		clearLines(adjustedHeight + offsetY, adjustedHeight + offsetY, currTerminalWidth);
		moveCursor((currTerminalWidth - 60)/ 2, adjustedHeight + offsetY);
		printf("Is the txt file in the same folder of the C program? [Y/N] ");
		confirm = getConfirm(currTerminalWidth, adjustedHeight, offsetY);

	} while (confirm != 'Y' && confirm != 'y' && confirm != 'N' && confirm != 'n');

	if (confirm == 'Y' || confirm == 'y') {
		do {
			clearLines(adjustedHeight + offsetY + 1, adjustedHeight + offsetY + 2, currTerminalWidth);
			moveCursor((currTerminalWidth - 38)/ 2, adjustedHeight + offsetY + 1);
...
void aboutProgram(int width, int height) {
	clearScreen();
	moveCursor((currTerminalWidth - 45) / 2, height / 4);
	...
	displayAbout(about, aboutCount, currTerminalWidth, height, 2);
	...
	displayAbout(about2, about2Count, currTerminalWidth, height, 7);
	...
	displayAbout(about3, about3Count, currTerminalWidth, height, 11);
	...
	displayAbout(members, membersCount, currTerminalWidth, height, 14);

	waitForDone(currTerminalWidth, height);
	clearScreen();
}
