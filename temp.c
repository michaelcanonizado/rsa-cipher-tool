...
BignumNode* createNewBignumNode(Bignum *num) {
    BignumNode *node = malloc(sizeof(BignumNode));
    node->value = num;
    node->previous = NULL;
    node->next = NULL;
    return node;
...
void initBignum(Bignum *num) {
    ...
    int *digitsPtr = calloc(DEFAULT_BIGNUM_LENGTH, sizeof(int));
...
void freeAllBignums() {
    ...
    while(tempNode != NULL) {
        tempNode = tempNode->next;
        printf("%llu - ", FREED_BIGNUMS_COUNT);
...
void freeBignum(Bignum *num) {
    ...
    if (bignumListHead->next == NULL) {
        printf("\nFreeing %p.%p", bignumListHead->value, bignumListHead->value->digits);
        if (bignumListHead->value->digits != NULL) {
            free(bignumListHead->value->digits);
        }
        free(bignumListHead);
    ...
    //     printf("\nFreeing %p.%p", bignumListHead->value, bignumListHead->value->digits);
    //     BignumNode *toRemove = bignumListHead;
    ...
    while(tempNode != NULL) {
        if (tempNode->value == num) {
            ...
            printf("\nFreeing %p.%p", tempNode->value, tempNode->value->digits);
