...
int millerRabinPrimalityTest(Bignum *num) {
    ...
    Bignum k, m, twoPowKTimesM, s, sModTwo;
    initBignum(&k);
    ...
    copyBignum(&s, &numMinusOne);
    moduloBignum(&sModTwo, &s, &two);

    while(isBignumZero(&sModTwo)) {
        // printf("\nS:");
        // printBignum(&s);

        if (!isBignumZero(&sModTwo)) {
            break;
        }
        halfBignum(&s, &s);
    ...
    Bignum res, temp, tempTemp, x, tempModTwo;
    initBignum(&res);
    initBignum(&temp);
    initBignum(&tempTemp);
    initBignum(&x);
    initBignum(&tempModTwo);
    copyBignum(&temp, &s);

    modularExponentiationBignum(&res, &a, &temp, num);

    while(!isEqualToBignum(&temp, &numMinusOne) && 
    !isEqualToBignum(&res, &pOne) && 
    !isEqualToBignum(&res, &numMinusOne)) {
        multiplyBignum(&x, &res, &res);
        moduloBignum(&res, &x, num);

        multiplyBignum(&tempTemp, &temp, &two);
        copyBignum(&temp, &tempTemp);
    }
    moduloBignum(&tempModTwo, &temp, &two);
    if (!isEqualToBignum(&res, &numMinusOne) && isBignumZero(&tempModTwo)) {
        printf("\nCOMPOSITE!");
        return 0;
    }
    printf("\nPRIME!");
    ...
    Bignum b, pOneModN, nOneModN;
    initBignum(&b);
    initBignum(&pOneModN);
    initBignum(&nOneModN);

    moduloBignum(&pOneModN, &pOne, num);
    moduloBignum(&nOneModN, &nOne, num);
    ...
    //     if (isEqualToBignum(&pOneModN, &b)) {
    //         printf("\n\nExiting Loop!");
    //         printf("\nResult: COMPOSITE (1)");
    //         break;
    //     } else if (isEqualToBignum(&nOneModN, &b)) {
    //         printf("\n\nExiting Loop!");
    //         printf("\nResult: PRIME (2)");
    //         break;
    //     } else if (isEqualToBignum(&pOne, &b)) {
    //         printf("\n\nExiting Loop!");
    //         printf("\nResult: COMPOSITE (3)");
    //         break;
    //     } else if (isEqualToBignum(&nOne, &b)) {
    //         printf("\n\nExiting Loop!");
    //         printf("\nResult: PRIME (4)");
    //         break;
    //     }
...
int generateRandomBignumInRange(Bignum *result, Bignum *min, Bignum *max) {
    ...
    if (isGreaterThanBignum(min, max)) {
        ...
        printf("\n\tFunction: generateRandomBignumInRange()");
        printf("\n\tExit code: -1\n\n\n");
    ...
    Bignum *referenceBignum = NULL;
    BIGNUM_SIGN randSign;
    unsigned long long int resultMaxLength = 0, resultLength = 0;

    if (min->sign != max->sign) {
        printf("\nSigns different! Choosing a random sign...");
        randSign = positive + rand() % (negative - positive + 1);

        resultMaxLength = min->sign == randSign ? min->length : max->length;

        referenceBignum = min->sign == randSign ? min : max;
    } else {
        printf("\nSign are the same...");
        randSign = max->sign;

        resultMaxLength = min->length > max->length ? min->length : max->length;

        referenceBignum = min->length == resultLength ? min : max;
    }

    resultLength = 1 + rand() % (resultMaxLength - 1 + 1);

    printf("\n\nrand sign: %d", randSign);
    printf("\n\nref len: %llu", referenceBignum->length);
    printf("\nmax length: %llu", resultMaxLength); 
    printf("\nchosen length: %llu", resultLength);
    ...
    for (; i > 0; i--) {
        // min + rand() % (max - min + 1);
        if (flag == 0 && i == resultLength - 1) {
            randDigit = 1 + rand() %(referenceBignum->digits[i] - 1 + 1);

            result->digits[i] = randDigit;
        } else if (flag == 0) {
            randDigit = rand() % 10;
            ...
            flag = randDigit != referenceBignum->digits[i] ? 0 : 1; 
        }
    }
